        ...
        <div className="w-12 p-3 px-0 grid justify-content-between">
          <div className="col-6 ">
            ...
            <div className="w-12 mt-2">
              <Carousel value={product?.images ? product?.images : ''} numVisible={3} numScroll={1} responsiveOptions={responsiveOptions} itemTemplate={productTemplate1} autoplayInterval={7000} circular showIndicators={false} className='mt-3' style={{ width: "100%" }} />
            </div>
          </div>
          <div className="col-6">
            ...
            <div className="pl-4 ">
              <Button label="ADD TO CART" className="w-12 text-white" severity="danger" />
            </div>
          </div>
        </div>
      </Dialog>
      <div className="no-underline text-pink-900 border-1 surface-border h-23rem border-round m-2 text-center py-5 px-3 flex flex-column justify-content-between relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onClick={handleClick}>
        <div className="mb-3 w-12 relative"  >
          {isShow && <div className='absolute bottom-0 w-12 flex justify-content-center gap-2 icon z-0' >
            <span title="Quick View" onClick={(e) => handleClickOptions(e, 'QUICK_VIEW')}><SelectOptions icon={<i class="fa-solid fa-eye"></i>} /></span>

            <span title="More information" onClick={(e) => handleClickOptions(e, 'MENU')}><SelectOptions icon={<i class="fa-solid fa-bars"></i>} /></span>
            <span title="Like" onClick={(e) => handleClickOptions(e, 'WISHLIST')}>{current?.wishlist?.some(i => i._id === product?._id) ? <SelectOptions icon={<i class="fa-solid fa-heart text-red-500" ></i>} /> : <SelectOptions icon={<i class="fa-solid fa-heart" ></i>} />}</span>
          </div>}
